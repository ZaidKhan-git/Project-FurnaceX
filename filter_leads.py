...
class LeadFilter:
        ...
        for state_dir in self.base_path.iterdir():
            ...
            for csv_file in state_dir.glob("*.csv"):
                try:
                    ...
                    self.all_leads.append(df)
                    print(f"  Loaded {len(df)} records from {state_dir.name}/{csv_file.name}")
                except Exception as e:
                    print(f"  Error loading {csv_file}: {e}")
        
        # Combine all dataframes
        self.combined_df = pd.concat(self.all_leads, ignore_index=True)
        
        # Remove duplicates based on Proposal No
        original_count = len(self.combined_df)
        self.combined_df = self.combined_df.drop_duplicates(subset=['Proposal No.'], keep='first')
        removed = original_count - len(self.combined_df)
    ...
    def export_filtered_leads(self, output_path, min_score=40):
        ...
        export_columns = [col for col in export_columns if col in filtered.columns]
        export_df = filtered[export_columns]
        ...
        print("="*60)
        print(f"Total leads exported: {len(export_df)}")
        print(f"\nBy Priority Tier:")
        print(export_df['Priority_Tier'].value_counts())
        print(f"\nBy State:")
